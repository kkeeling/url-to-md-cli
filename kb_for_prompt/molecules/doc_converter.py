"""
Word document to Markdown converter module.

This module provides functionality to convert Word documents (.doc and .docx files) 
to Markdown format using the docling library. It includes file validation, 
error handling, and retry mechanisms for reliable conversion.
...
Example:
    ...
    markdown_content, original_path = convert_doc_to_markdown(
...
def convert_doc_to_markdown(
    ...
    """
    Convert a Word document (.doc or .docx) to markdown content using docling.
    ...
    Args:
        file_path: The path to the Word document (str or Path object)
    ...
    Raises:
        ValidationError: If the file path is invalid or the file is not a Word document
