...
class TestCliCondensationIntegration:
    ...
    def test_integration_success_path(self, mock_converter_cls, mock_confirm_ask, mock_condense, mock_spinner, tmp_path):
        ...
        # Create a dummy input file to avoid potential issues if the converter tries to access it
        input_file.touch()
        ...
        mock_condense.assert_called_once_with(Path(converted_file_path)) # Ensure Path object is passed
        assert "✓ Conversion successful." in result.output
        assert f"Output file: [cyan]{converted_file_path}[/cyan]" in result.output # Check rich markup
        assert "✓ Condensation successful." in result.output
        assert f"Condensed file: [cyan]{condensed_file_path}[/cyan]" in result.output # Check rich markup
    ...
    def test_integration_condensation_returns_none(self, mock_converter_cls, mock_confirm_ask, mock_condense, mock_spinner, tmp_path):
        ...
        mock_condense.assert_called_once_with(Path(converted_file_path)) # Condensation is attempted
        assert "✓ Conversion successful." in result.output
        assert f"Output file: [cyan]{converted_file_path}[/cyan]" in result.output
    ...
    @patch('kb_for_prompt.pages.kb_for_prompt.display_spinner') # Mock spinner as it shouldn't be called
    @patch('kb_for_prompt.pages.kb_for_prompt.condense_knowledge_base')
    @patch('kb_for_prompt.pages.kb_for_prompt.Confirm.ask', return_value=False) # Mock user declining
    @patch('kb_for_prompt.pages.kb_for_prompt.SingleItemConverter')
    def test_integration_user_declines_condensation(self, mock_converter_cls, mock_confirm_ask, mock_condense, mock_spinner, tmp_path):
        """
        Test successful conversion, but user declines the condensation step.
        """
        # --- Arrange ---
        input_file = tmp_path / "input.txt"
        output_dir = tmp_path / "out"
        ...
        # --- Assert ---
        # Exit code should be 0 because the initial conversion succeeded
        assert result.exit_code == 0, f"CLI exited with code {result.exit_code}. Output:\n{result.output}"
        mock_converter_cls.assert_called_once()
        expected_output_path_resolved = output_dir.resolve()
        mock_converter_instance.run.assert_called_once_with(str(input_file), expected_output_path_resolved)
        ...
        # Ensure condensation steps were NOT taken because user declined
        ...
        assert "✓ Conversion successful." in result.output
        assert f"Output file: [cyan]{converted_file_path}[/cyan]" in result.output
        assert "Condensation successful" not in result.output # No condensation success message
        assert "Condensation failed" not in result.output    # No condensation failure message
